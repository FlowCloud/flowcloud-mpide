...
typedef struct
{
    // TOP level response control parameters
    uint32_t    bytesReadCount;                 // Counter of current response already reading
    size_t      numBytes;                       // Number of bytes of the current response
    uint8_t     responseTxDone; 
    uint8_t     padding;                        // padding field to have structure multiple of 32 bits 
} RESPONSE_CTRL;
...
typedef struct
{
	...
	RESPONSE_CTRL			TxResponse;                     // Current sending response stub
    HTTP_STATUS				httpStatus;                     // Request method/status
    HTTP_CONTENTTYPE_TYPE	fileType;                       // File type to return with Content-Type
    SM_HTTP2				sm;                             // Current connection state
    SM_FILETX				file_sm;                        // Current file sending state
	SM_RESPONSETX			response_sm;                    // Current response-sending state
	...
	HTTP_REQUEST_INFO*		requestInfo;					// Infomation about the current request for this connection
	HTTP_RESPONSE_INFO*		responseInfo;					// Infomation about the current response for this connection

} HTTP_CONN;


// Handler for an endpoint
typedef void (*GetEndPointHandler)(HTTP_CONN* pHttpCon);
typedef HTTP_IO_RESULT (*PostEndPointHandler)(HTTP_CONN* pHttpCon);
